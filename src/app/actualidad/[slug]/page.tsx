...
export async function generateMetadata({
    ...
    const post = await client.fetch<Post>(POST_QUERY, { slug: slug }, options);
    const postImageUrl = post?.image ? urlFor(post.image, 1200)?.url() : null;

    return {
        title: post?.title ?? "Publicación | Demosmap",
        description: post?.description ?? "Lee esta publicación en Demosmap.",
        openGraph: {
            type: "article",
            title: post?.title,
            ...
            images: postImageUrl
                ...
                : [],
            url: `https://demosmap.com/actualidad/${slug}`,
        },
        twitter: {
        ...
        alternates: {
            canonical: `https://demosmap.com/actualidad/${slug}`,
        },
    };
}
